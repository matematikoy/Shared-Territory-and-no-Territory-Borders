warn("Thank you for using the 'Shared Territory and no Territory Borders' mod."); 
...
Capturable.prototype.TimerTick = function()
{
	...
	if (cmpTerritoryDecay && cmpTerritoryDecay.IsDecaying())
	{
		...
		let cmpDiplomacy = QueryPlayerIDInterface(owner, IID_Diplomacy);

		let totalNeighbours = neighbours.reduce((sum, val, i) =>
			(i != 0 && cmpDiplomacy && cmpDiplomacy.IsEnemy(i)) ? sum + val : sum, 0);
		...
		// Decay para Gaia como fallback (mantido, mas opcional)
		else
		{
	...
	delete this.timer;
	Engine.PostMessage(this.entity, MT_CaptureRegenStateChanged, {
		"regenerating": false,
		"regenRate": 0,
		"territoryDecay": 0
	});
